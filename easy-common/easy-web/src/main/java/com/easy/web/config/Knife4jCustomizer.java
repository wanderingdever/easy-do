...
public class Knife4jCustomizer implements PropertyCustomizer {
    ...
    public Schema<?> customize(Schema schema, AnnotatedType annotatedType) {
        ...
        for (Annotation ctxAnnotation : ctxAnnotations) {
            ...
            // 可使用列表
            Set<String> allowList = CollUtil.isEmpty(schema.getEnum()) ? new HashSet<>() : new HashSet<String>(schema.getEnum());
            ...
            Object[] enumArray = Arrays.stream(enums)
                    .filter(k -> CollectionUtils.isEmpty(schema.getEnum()) || allowList.contains(k.name()))
                    .toArray();
            ...
            if (CollUtil.isNotEmpty(descList)) {
                ...
                log.warn(clazz.getName() + " 未配置描述或可选值不在枚举取值范围内");
            }
